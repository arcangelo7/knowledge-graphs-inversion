...
R2RML allows assigning generated triples to named graphs through `rr:graphMap`, which can use the same constant, reference, and template types as other term maps. A constant graph map (e.g., `rr:graph ex:PersonGraph`) does not reference any column and has no effect on reconstruction: the triples are simply retrieved from the named graph. A template or reference graph map encodes column values in the graph IRI. When these columns also appear in other term maps (subject, predicate, or object), the algorithm recovers them from there without additional extraction. When a column is referenced exclusively by the graph map, the algorithm adds one `GRAPH ?g { ... }` clause per graph map, matching a triple pattern of the predicate-object map that graph map belongs to, and applies the same `STRAFTER`/`STRBEFORE` extraction to each graph variable. The clauses join on the subject variable, so columns coming from different named graphs land on the same reconstructed row. Graph maps that build IRIs with the same pattern cannot be told apart, because a graph IRI does not say which graph map produced it. Columns that only those graph maps expose are recoverable as a set but not attributable to a column, so they are left out of the reconstruction. See [limitations](limitations).
