...
class DatabaseSchemaRetriever:
    ...
    def _sql_to_python_type(self, sql_type) -> type:
        ...
        if isinstance(sql_type, (sqlalchemy.Integer, sqlalchemy.BigInteger, sqlalchemy.SmallInteger)):
            return int
        elif isinstance(sql_type, (sqlalchemy.Float, sqlalchemy.Numeric, sqlalchemy.DECIMAL)):
            return float
...
def infer_type_from_value_with_schema(value: Any, column_info: Optional[ColumnInfo] = None) -> Any:
    """
    ...
    try:
        ...
        if column_info.python_type == int:
            return int(float(value)) if value is not None else None
        elif column_info.python_type == float:
            return float(value) if value is not None else None
        elif column_info.python_type == bool:
