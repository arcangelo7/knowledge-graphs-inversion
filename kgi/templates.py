...
class CSVTemplate(Template):
    ...
    def create_template(self) -> str:
        ...
        return (
            "No real CSV template is created as we can just dump the dataframe to csv"
        )
...
class Object(Node):
    ...
    def __init__(
        self, children: dict[str, Node] | None = None, values: list[str] | None = None
    ):
        self._parent_path = ""
    ...
    def to_template(self) -> str:
        ...
        child_strings = [
            f'"{key}": {child.to_template()}' for key, child in self.children.items()
        ]
    ...
    def fill(self, data: pd.DataFrame) -> str:
        ...
        paths = [
            JSONPathFunctions.normalize_json_path(f"{self.path}.['{value}']")
            for value in self.values
        ]
        filled_values = [
            f'"{value}": "{data[path].iloc[0]}"'
            for value, path in zip(self.values, paths)
        ]
        filled_children = [
            f'"{key}": {child.fill(data)}' for key, child in self.children.items()
        ]
