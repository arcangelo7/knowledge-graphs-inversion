...
| Two column variants             | R2RMLTC0002f                                | RMLTC0002f**       | Forward mapping fails in RML (case sensitivity bug)                                                            |
| Two column variants             | R2RMLTC0002i-j                              | RMLTC0002i-j*      | R2RML uses `rr:sqlQuery`; RML uses `rml:query`. Both are error test cases (empty `output.nq`)                  |
...
| Composite keys, ref object maps | R2RMLTC0008b                                | RMLTC0008b*        | morph-kgc fails to produce the RefObjectMap triple (`ex:Sport`) from the RML mapping (4 triples instead of 5); R2RML works correctly. Forward mapping bug, not an inversion difference |
